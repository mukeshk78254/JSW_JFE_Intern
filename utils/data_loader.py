...
#         # BULLETPROOF MAPPING: Search by exact Excel Column Index based on your image
#         # If the columns change names/formats, this grabs them based on their exact position
...
#         # Locate and Process Date Column Safely
#         if 'DATE' in df_bops.columns:
#             # First, drop rows where Date is purely NaN (removes sub-header rows instantly)
#             df_bops = df_bops.dropna(subset=['DATE'])
#             # Convert to DateTime (coerces bad text like 'AVG. TEMP' or 'DATE' headers to NaT)
#             df_bops['Timestamp'] = pd.to_datetime(df_bops['DATE'], errors='coerce')
#             # Drop the rows that failed conversion (removes duplicated header rows)
...
#         # Force Numeric Types safely (converts text/hyphens to NaN, then fills NaN with 0.0)
#         for col in ['Casting Speed', 'Production', 'Tundish Temp', 'Lifting Temp', 'Casting Time']:
#             if col in df_bops.columns:
#                 # Convert the column to string first, replace hyphens, then make numeric
...
def load_all_data():
    ...
    # Locate files based on naming conventions
    bops_file = find_file("*BOPS*.xlsx", possible_dirs) or find_file("*log*sheet*.xlsx", possible_dirs)
    csp_delay = find_file("*CSP DELAY*.xlsx", possible_dirs)
    elec_delay = find_file("*Electrical Delay*.xlsx", possible_dirs)
    chem_file = find_file("*Chemistry*.xlsx", possible_dirs)
    ...
    try:
        ...
        # EXACT MATCHING with Line-Break, Space, and Period removal
        def get_col(keyword):
            clean_key = keyword.upper().replace(" ", "").replace(".", "")
            ...
            # Check first 3 rows for sub-headers
            if not df_bops.empty:
                for idx in range(min(3, len(df_bops))):
                    for col in df_bops.columns:
                        clean_cell = str(df_bops.iloc[idx][col]).upper().replace("\n", "").replace(" ", "").replace(".", "")
                        if clean_key in clean_cell: return col
            return None

        # Properly loop through and apply the rename map
        rename_map = {}
        target_cols = [
            ('SHIFT', 'Shift'),
            ('HEATNO', 'Heat Number'),
        ...
        for search_key, target_name in target_cols:
            matched_col = get_col(search_key)
            if matched_col is not None:
                rename_map[matched_col] = target_name
        ...
        date_col = next((c for c in df_bops.columns if 'DATE' in str(c).upper()), None)
        if date_col:
            # Drop rows where Date is NaN (this instantly deletes the sub-header rows)
            df_bops = df_bops.dropna(subset=[date_col])
            df_bops['Timestamp'] = pd.to_datetime(df_bops[date_col], errors='coerce')
            # Drop rows that failed to convert to a date (cleans up any remaining text rows)
            df_bops = df_bops.dropna(subset=['Timestamp']).reset_index(drop=True)
        ...
        # Force Numeric Types safely (Also handles the hyphens "-" in Excel)
        for col in ['Casting Speed', 'Production', 'Tundish Temp', 'Lifting Temp', 'Casting Time']:
            if col in df_bops.columns:
                df_bops[col] = df_bops[col].astype(str).str.replace('-', '0', regex=False)
                df_bops[col] = pd.to_numeric(df_bops[col], errors='coerce').fillna(0)
